...
function complete_grading($data)
{
    //count sched
    $sched_count = count($data);
    //count graded
    $sched_graded = 0;

    foreach ($data as $key => $row)
    {
        $sched_graded += $row->is_graded;
    }

    if($sched_count == $sched_graded)
     return TRUE;
    else
     return FALSE;
...
function is_valid_grade($input)
{
    ...
    $key = array_search($input, $valid_grade);

    return in_array((string) $input, $valid_grade) ? $valid_grade[$key] : FALSE;
}

function get_column($array, $field)
{
    $data = array();
    foreach ($array as $item) $data[] = get_key($item, $field);
    return $data;
}
