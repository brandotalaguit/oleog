...
<article class="teaching-load-encode">
	...
	<hr>
	<p>
	<strong>REMINDER</strong>: Please <strong>DOUBLE CHECK</strong> the grades of your students <strong>BEFORE</strong> clicking the <strong>SAVE</strong> button.</p>
	</div>
		...
		<?php
						...
						foreach ($students as $row) :
									...
									if ($row->enabled == 1)
									{
										...
										$new_old = $row->new_grade_sys == 1 ? 'New' : 'Old';
										$input .= 'data-toggle="tooltip" data-placement="top" title="Input ' . $new_old . ' Grading System"';
										$input .= " value='{$row->Grade}' maxlength='5' char='5' />";
										$ctr++;
										$inputleclab = "<input type='text' name='labgrade[]' class='form-control input-mini LabGrade' tabindex='{$ctr}' ";
										$inputleclab.= 'data-toggle="tooltip" data-placement="top" title="Input ' . $new_old . ' Grading System"';
										$inputleclab.= " value='{$row->LabGrade}' maxlength='5' char='5' />";
										// $ctr++;
									}
									else
									{
										...
										$inputleclab  = "<input type='hidden' name='labgrade[]' value='{$row->LabGrade}' />";
										$inputleclab.= "<span class='uneditable-input form-control' disabled>{$row->LabGrade}</span>";
									}
									...
									$outputleclab = "<input type='hidden' name='LabStrGrade[]' value='{$row->StrLab}' class='StrLab' />";
									$outputleclab.= "<span class='uneditable-input outputleclab form-control' disabled>{$row->StrLab}</span>";
		?>
						...
						<tr>
						  ...
						  <td width="5%">
							...
							<td width="5%">
								<?php echo $inputleclab; ?>
							</td>
							<td width="5%">
								<?php echo $outputleclab; ?>
							</td>
							<td width="15%">
								<span class="">
